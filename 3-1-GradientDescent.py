...
# 正态分布生成随机数据，有两个特征向量，则按照二维正态分布生成数据，mean=[*, *]为正态分布的均值参数；cov=[[*, *],[*, *]]为正态分布的协方差矩阵参数
x1 = np.random.multivariate_normal([0, 0], [[1, .75],[.75, 1]], num_observations)
x2 = np.random.multivariate_normal([1, 4], [[1, .75],[.75, 1]], num_observations)
# np.vstack((x1, x2)) 按垂直方向堆垒构成新数组，即按照一行一行叠加，相对应的是np.hstack,按列一列一列叠加；
...
def logistic_regression_minibatch(X, y, num_steps, learning_rate):
    ...
    for step in range(num_steps):
        ...
        # TODO 对于w, b的梯度计算
        # grad_w = np.sum(np.dot(sigmoid(np.dot(X_batch, w) + b) - y_batch, X_batch))
        # grad_b = np.sum((sigmoid(np.dot(X_batch, w) + b) - y_batch))
